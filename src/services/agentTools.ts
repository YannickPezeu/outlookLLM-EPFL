...
export const PRESERVED_TOOLS = new Set<string>([
  ...
  // Streaming-summary tools: their full output (summary/briefing) is generated by
  // an INTERNAL LLM call streamed straight to the UI (already_displayed:true), so
  // it never lands in the agent's assistant text (the anti-repetition rule keeps
  // the agent's reply to a short transition). The only copy of that text lives in
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "• AVEC query → trie les emails par pertinence sémantique (embeddings) et te retourne leurs refs+sujets+CORPS " +
        "(extrait ~2000 car. du contenu — tu peux donc LIRE l'email et en extraire une info précise, ex: une URL, un montant) " +
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "Le paramètre 'sender' permet de combiner expéditeur + contenu en une fois (ex: 'from:matéo docling') : " +
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Le texte / mots-clés à rechercher dans les emails",
          },
          sender: {
            type: "string",
            description:
              "Optionnel : restreint aux emails REÇUS d'un expéditeur (nom ou adresse email). " +
              "Combiné avec query dans la même recherche (KQL from:). " +
              "Ex: sender='Matéo Muller' ou sender='mateo@epfl.ch'. " +
              "Utilise-le quand l'utilisateur dit « l'email de X qui parle de Y », « ce que X m'a envoyé sur Z ».",
          },
          max_results: {
            type: "number",
...
// How much cleaned body to hand back per email in search results. A 200-char
// bodyPreview is near-useless for content lookup (a URL, a figure, a decision
// sits well past it), so search tools now return a real slice of the body — the
// agent can read the content directly instead of being blind to it.
const BODY_SNIPPET_CHARS = 2000;

/**
 * Build a readable body snippet for a search result: strip HTML, keep the full
 * thread (so quoted content like a forwarded URL survives), truncate. Falls back
 * to the Graph bodyPreview when no full body is available.
 */
function bodySnippet(rawBody: string | undefined, fallbackPreview?: string): string {
  const cleaned = cleanEmailBodyFull(rawBody || "");
  const text = cleaned || (fallbackPreview || "");
...
const executors: Record<string, ToolExecutor> = {
  ...
  async get_email_interactions(args, log) {
    ...
    // Date scope (BOTH modes): honour an explicit range, otherwise ALL-TIME.
    // A content query must NOT shrink the time window. A prior 6-month default on
    // the query path silently hid older matches — e.g. "l'email de Matéo sur
    // docling" (Aug 2025) returned nothing while the global search_emails found it,
    // purely because naming the contact triggered a 6-month bound the no-query mode
    // never applied. The pool stays bounded by MAX_PER_DIRECTION below and the
    // 500-email embedding cap, so all-time is safe (network + embedding cost stay
    // capped, only the *recency* of what's kept changes).
    ...
    // Return the full ranked set with a real body snippet — so the agent can read
    // the content (e.g. pull a URL out of the matching email) instead of guessing
    // from a preview. Bodies are ALREADY in memory here (getAllInteractions selects
    // body), so this costs no extra fetch; topEmails is already capped to ≤100 by
    // the ranking above, well within the context budget.
    const richEmails = [...topEmails];
    richEmails.sort((a, b) => new Date(b.displayDate).getTime() - new Date(a.displayDate).getTime());

    for (const e of richEmails.slice(0, 10)) {
    ...
    const emailSummaries = richEmails.map((e) => ({
      ...
      from: e.direction === "sent"
        ? "Moi"
        : (e as any).from?.emailAddress?.name || (e as any).from?.emailAddress?.address || "?",
      body: bodySnippet((e as any).body?.content, e.bodyPreview),
  ...
  async search_emails(args, _log) {
    const query = args.query as string;
    const sender = (args.sender as string | undefined)?.trim() || undefined;
    const maxResults = (args.max_results as number) || 100;
    const dateRange = extractDateRange(args);
    const emails = await searchEmails(query, maxResults, dateRange, sender);

    // searchEmails now projects `body`, so each row already carries its content
    // (one paginated request, no per-email fetch). Return a cleaned snippet so
    // the agent can read the email and extract a precise info (URL, amount, …).
    const results = emails.map((e) => ({
      ...
      body: bodySnippet(e.body?.content, e.bodyPreview),
      has_attachments: !!(e as any).hasAttachments,
    }));
  ...
  async read_email_attachments(args, log) {
    ...
    const texts = await extractTextFromAttachments(attachments as any, undefined, {
      onProgress: log,
    });
